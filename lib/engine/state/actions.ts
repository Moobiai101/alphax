...
import type {TEXT_GRADIENT, XTrack} from "./types"
import {withBroadcast} from "../utils/utils/with-broadcast"
import {Transition} from "../controllers/controllers/compositor/parts/transition-manager"
import {Filter, FilterType} from "../controllers/controllers/compositor/parts/filter-manager"
import {actionize_historical, actionize_non_historical} from "../utils/utils/actionize"
import {
	AnyEffect,
	AudioEffect,
	ExportStatus,
...
import {Animation, AnimationFor} from "../controllers/controllers/compositor/parts/animation-manager"
...
export const historical = actionize_historical({
	toggle_track_muted: state => (trackId: string) => {
		const track = state.tracks.find((track: XTrack) => track.id === trackId)
		if(track) {
	...
	remove_transition: state => (id: string) => {
		state.transitions = state.transitions.filter((t: Transition) => t.id !== id)
	},
	set_transition_duration: state => (duration: number, transitionId: string) => {
		const effect = state.transitions.find((t: Transition) => t.id === transitionId)
		if(effect)
	...
	set_animation_duration: state => (duration: number, {id}: VideoEffect | ImageEffect) => {
		const effect = state.animations.find((a: Animation) => a.targetEffect.id === id)
		if(effect)
			effect.duration = duration
	},
	...
	remove_animation: state => (effect: VideoEffect | ImageEffect, type: "in" | "out", animationFor: AnimationFor) => {
		state.animations = state.animations.filter((animation: Animation) => !(animation.targetEffect.id === effect.id && animation.type === type && animation.for === animationFor))
	},
	remove_filter: state => (effect: ImageEffect | VideoEffect, type: FilterType) => {
		state.filters = state.filters.filter((filter: Filter) => !(filter.targetEffectId === effect.id && filter.type === type))
	},
	...
	set_pivot: state => ({id}: VideoEffect | ImageEffect | TextEffect, x: number, y: number) => {
		const effect = state.effects.find((e: AnyEffect) => e.id === id) as VideoEffect | ImageEffect
		effect!.rect.pivot = {x, y}
	},
	set_text_fill: state => ({id}: TextEffect, color: string, index: number) => {
		const effect = state.effects.find((e: AnyEffect) => e.id === id) as TextEffect
	...
	remove_fill_gradient_stop: state => ({id}: TextEffect, index: number) => {
		const effect = state.effects.find((e: AnyEffect) => e.id === id) as TextEffect
		effect.fillGradientStops = effect.fillGradientStops.filter((_: number, i: number) => i !== index)
	},
	...
	remove_text_fill: state => ({id}: TextEffect, index: number) => {
		const effect = state.effects.find((e: AnyEffect) => e.id === id) as TextEffect
		effect.fill = effect.fill.filter((_: unknown, i: number) => i !== index)
	},
	...
	remove_track: state => (id: string) => {
		const new_arr = state.tracks.filter((track: XTrack) => track.id !== id)
	...
	set_rotation: state => ({id}: TextEffect | ImageEffect | VideoEffect, rotation: number) => {
		const effect = state.effects.find((e: AnyEffect) => e.id === id) as TextEffect | VideoEffect | ImageEffect
	...
	set_effect_width: state => ({id}: TextEffect | ImageEffect | VideoEffect, width: number) => {
		const effect = state.effects.find((e: AnyEffect) => e.id === id) as Exclude<AnyEffect, AudioEffect>
	...
	remove_effect: state => ({id}: AnyEffect) => {
		const effects = state.effects.filter((e: AnyEffect) => e.id !== id)
		state.effects = effects
	},
	set_effect_scale: state => (effect: TextEffect | ImageEffect | VideoEffect, scale: {x: number, y: number}) => {
		const eff = state.effects.find((e: AnyEffect) => effect.id === e.id) as TextEffect | VideoEffect | ImageEffect
...
// Define action types
type HistoricalActions = ZipAction.Callable<typeof historical>
type NonHistoricalActions = ZipAction.Callable<typeof non_historical>
export type Actions = HistoricalActionsWithBroadcast & NonHistoricalActionsWithBroadcast

let broadcaster: any = null;
export const setBroadcaster = (b: any) => { broadcaster = b; };

// Wrapped actions - use type assertion to work around complex ZipAction type inference
// The runtime behavior is correct, but TypeScript cannot infer the correct intersection type
export const historical_actions: HistoricalActionsWithBroadcast = (Object.keys(historical) as (keyof typeof historical)[]).reduce(
	(acc, key) => {
		const action = historical[key]
		acc[key] = withBroadcast(action, (actionType, payload) => {
			broadcaster?.broadcastAction(actionType as keyof Actions, payload)
		}) as any
	return acc
	},
	{} as any
...
export const non_historical_actions: NonHistoricalActionsWithBroadcast = (Object.keys(non_historical) as (keyof typeof non_historical)[]).reduce(
	(acc, key) => {
		const action = non_historical[key]
		acc[key] = withBroadcast(action, (actionType, payload) => {
			broadcaster?.broadcastAction(actionType as keyof Actions, payload)
		}) as any
	return acc
