...
import {FFmpegHelper} from "../helpers/FFmpegHelper/helper"

export class Encoder {
	#encode_worker: Worker | null = null
	#ffmpeg: FFmpegHelper
	...
	// Lazy worker initialization to avoid Turbopack resolving at build time
	get encode_worker(): Worker {
		if (!this.#encode_worker) {
			if (typeof window === 'undefined') {
				throw new Error('Encoder worker can only be created in browser environment')
			}
			this.#encode_worker = new Worker(new URL("./encode_worker.ts", import.meta.url), {type: "module"})
		}
		return this.#encode_worker
	}

	reset() {
		if (this.#encode_worker) {
			this.#encode_worker.terminate()
			this.#encode_worker = null
		}
		this.file = null
	}
