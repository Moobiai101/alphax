...
import type {Collaboration} from "../controller"
import {AnyMedia} from "../../../../types/media-types"
import {BinaryAccumulator} from "../../video-export/tools/BinaryAccumulator/tool"
...
export class FileHandler {
			...
			if (onProgress && this.#fileProgress[hash]) {
				...
				if(proxy) {
					...
					if(received >= total) {
						...
						const binaryData = this.binary_accumulators[hash].binary
	...
	#sendChunkWithHash(dataChannel: RTCDataChannel, chunk: ArrayBuffer | Uint8Array, hash: string) {
		const encoder = new TextEncoder()
		const hashBuffer = encoder.encode(hash)
		
		// Convert Uint8Array to ArrayBuffer if needed
		const chunkBuffer = chunk instanceof Uint8Array ? chunk.buffer.slice(chunk.byteOffset, chunk.byteOffset + chunk.byteLength) : chunk
		const chunkView = chunk instanceof Uint8Array ? chunk : new Uint8Array(chunk)

		const combinedBuffer = new ArrayBuffer(hashBuffer.byteLength + chunkBuffer.byteLength)
		const combinedView = new Uint8Array(combinedBuffer)

		combinedView.set(new Uint8Array(hashBuffer), 0)
		combinedView.set(chunkView, hashBuffer.byteLength)
