console.log("[COMPRESSOR] Module loading started");
import {generate_id} from "@benev/slate/x/tools/generate_id"
console.log("[COMPRESSOR] generate_id imported");

import {State} from "../../../../state/types"
console.log("[COMPRESSOR] State imported");
import {omnislate} from "../../../../omnislate"
console.log("[COMPRESSOR] omnislate imported");
import type {Collaboration} from "../controller"
console.log("[COMPRESSOR] Collaboration imported");
import {demuxer} from "../../../../tools/tools/demuxer"
console.log("[COMPRESSOR] demuxer imported");
import {VideoFile} from "../../../../types/media-types"
console.log("[COMPRESSOR] All imports complete");
...
export class Compressor {
	...
	#createDecoderWorker() {
		...
		const worker = new Worker(new URL("../../video-export/parts/decode_worker.ts", import.meta.url), {type: "module"})
	...
	#createEncoderWorker() {
		const id = generate_id()
		const worker = new Worker(new URL("../../video-export/parts/encode_worker.ts", import.meta.url), {type: "module"})
