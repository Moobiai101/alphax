...
import {EffectDrop} from "./drag-related/effect-drag"
import {Compositor} from "../../compositor/controller"
import {Transition} from "../../compositor/parts/transition-manager"
import {AnyEffect, At, ProposedTimecode, State} from "../../../../state/types"
import {get_effects_at_timestamp} from "../../video-export/utils/get_effects_at_timestamp"
...
export class EffectManager {
	...
	removeEffect(state: State, effect: AnyEffect) {
		...
		this.compositor.managers.transitionManager.getTransitions()
			.filter((transition: Transition) => transition.incoming.id === effect.id || transition.outgoing.id === effect.id)
				.forEach((transition: Transition) => this.compositor.managers.transitionManager.removeTransition(transition.id))
