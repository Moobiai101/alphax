...
import {GLTransition} from "../../../../state/global"
...
import {normalizeTransitionDuration} from '../../../../../utils/normalize-transition-duration'
...
export class TransitionManager {
	...
	async #selectTransition(transition: Transition, state: State, recreate?: boolean) {
		...
		if (incoming && outgoing) {
			...
			const update = () => {
				...
				if(t) {
					const incoming = omnislate.context.state.effects.find((e: AnyEffect) => e.id === t.incoming.id)!
	...
	getTransition(id: string | null) {
		return omnislate.context.state.transitions.find((t: Transition) => t.id === id)
	}

	getTransitionByEffect(effect: AnyEffect) {
		return omnislate.context.state.transitions.find((t: Transition) => t.incoming.id === effect.id || t.outgoing.id === effect.id)
	}

	protected getAnimations(effect: AnyEffect) {
		return omnislate.context.state.transitions.filter((a: Transition) => a.incoming.id === effect.id)
	}
	...
	getTransitions () {
		return omnislate.context.state.transitions.map((a: Transition) => ({
			...a,
	...
	getTransitionByPair(outgoing: TransitionAbleEffect, incoming: TransitionAbleEffect) {
		return omnislate.context.state.transitions.find((t: Transition) => t.incoming.id === incoming.id && t.outgoing.id === outgoing.id)
	}
	...
	removeTransitionFromNoLongerTouchingEffects(state: State) {
		...
		omnislate.context.state.transitions.map((transition: Transition) => {
