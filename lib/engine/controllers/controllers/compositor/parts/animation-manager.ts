...
import {AnyEffect, ImageEffect, State, VideoEffect} from "../../../../state/types"
import {calculateProjectDuration} from "../../../../utils/utils/calculate-project-duration"
import type {
...
export class AnimationManager {
	...
	protected getAnimations(effect: AnyEffect) {
		return omnislate.context.state.animations.filter((a: Animation) => a.targetEffect.id === effect.id && a.for === this.animationFor)
	}

	protected getAnimation(effect: AnyEffect, kind: "in" | "out") {
		return omnislate.context.state.animations.find((a: Animation) => a.targetEffect.id === effect.id && a.type === kind && this.animationFor === a.for)
	}

	protected getAnyAnimation(effect: AnyEffect) {
		return omnislate.context.state.animations.find((a: Animation) => a.targetEffect.id === effect.id && a.for === this.animationFor)
	}
	...
	#onAnimationUpdate(object: PIXIContainer, animation: Animation) {
	...
	#resetObjectProperties(object: PIXIContainer, effect: ImageEffect | VideoEffect) {
	...
	removeAnimations(effect: AnyEffect) {
		...
		animations.forEach((a: Animation) => this.deselectAnimation(a.targetEffect, a.type))
	}
	...
	#killAnimationTweens(object: PIXIContainer) {
			...
			gsap.ticker.add(() => {
				...
				if (object) {
					...
					if (!refresh) {
						...
						if(object.mask instanceof PIXIContainer) {
							object.mask.destroy()
							object.mask = null
						}
					}
	...
	#handleSlideIn(object: PIXIContainer, effect: ImageEffect | VideoEffect, animation: Animation, tweenDuration: number) {
	...
	#handleSlideOut(object: PIXIContainer, effect: ImageEffect | VideoEffect, animation: Animation, tweenDuration: number) {
	...
	#handleFadeIn(object: PIXIContainer, effect: ImageEffect | VideoEffect, animation: Animation, tweenDuration: number) {
		const tween = gsap.fromTo(
			object,
			{
				alpha: 0,
	...
	#handleFadeOut(object: PIXIContainer, effect: ImageEffect | VideoEffect, animation: Animation, tweenDuration: number) {
		const tween = gsap.fromTo(
			object,
			{
				alpha: 1,
	...
	#handleSpinIn(object: PIXIContainer, effect: ImageEffect | VideoEffect, animation: Animation, tweenDuration: number) {
		const tween = gsap.fromTo(
			object,
			{
				animationName: animation.type,
	...
	#handleSpinOut(object: PIXIContainer, effect: ImageEffect | VideoEffect, animation: Animation, tweenDuration: number) {
		const tween = gsap.fromTo(
			object,
			{
				animationName: animation.type,
	...
	#handleBounceIn(object: PIXIContainer, effect: ImageEffect | VideoEffect, animation: Animation, tweenDuration: number) {
	...
	#handleBounceOut(object: PIXIContainer, effect: ImageEffect | VideoEffect, animation: Animation, tweenDuration: number) {
		const startLeft = effect.rect.position_on_canvas.x
	...
	#handleWipeIn(object: PIXIContainer, effect: ImageEffect | VideoEffect, animation: Animation, tweenDuration: number) {
		...
		const tween = gsap.to(dummy, {
			...
			data: { animationFor: this.animationFor },
			onUpdate: () => {
				if(object.mask instanceof Graphics) {
					object.mask.clear()
	...
	#handleWipeOut(object: PIXIContainer, effect: ImageEffect | VideoEffect, animation: Animation, tweenDuration: number) {
		const fullWidth = object.width
		const fullHeight = object.height
	...
	#handleBlurIn(object: PIXIContainer, effect: ImageEffect | VideoEffect, animation: Animation, tweenDuration: number) {
	...
	#handleBlurOut(object: PIXIContainer, effect: ImageEffect | VideoEffect, animation: Animation, tweenDuration: number) {
		const blurFilter = new BlurFilter()
		blurFilter.blur = 0
		blurFilter.quality = 10
		//@ts-ignore
	...
	#handleZoomIn(object: PIXIContainer, effect: ImageEffect | VideoEffect, animation: Animation, tweenDuration: number) {
		const tween = gsap.fromTo(
			object.scale,
			{ x: 0.4, y: 0.4 },
			{
	...
	#handleZoomOut(object: PIXIContainer, effect: ImageEffect | VideoEffect, animation: Animation, tweenDuration: number) {
