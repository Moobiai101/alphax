...
export const useEditorStore = create<EditorState>()(
  persist(
    (set, get) => ({
      // Initial states - these are NOT persisted, so they reset on each page load
      isInitializing: true,
      isPanelsReady: false,
      ...
      initializeApp: async () => {
        ...
        if (!get().isInitializing && get().isPanelsReady) {
          console.log("Video editor already initialized");
          return;
        }
